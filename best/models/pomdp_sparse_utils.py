...
def update_distribution(pomdp, D_ux, u_dim=None, x_dim=None):
...
def update_network_distribution(network, D):
    ...
    for name, pomdp in network.forward_iter():
      ...
      # D_ux -> D_xz
      D = update_distribution(pomdp, D, u_dim=u_dim, x_dim=x_dim)   # get old  z_out
...
def evaluate_Q(network, m_tuple, n_tuple, W):
    ...
    # sum over outputs (irrelevant for value)
    D_x = D_xz.sum(axis = range(len(network.N), len(network.N) + len(network.O))) 

    ret = 0.
    for i in range(len(D_x.data)):
      midx = tuple(D_x.coords[k][i] for k in range(len(D_x.shape)))
      ret += D_x.data[i] * W[midx]

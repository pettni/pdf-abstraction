...
# p3 = rf.vertex_to_poly(np.array([[1, -2], [1, 2], [2.5, 2], [2.5, -2]]))
# regs['r3'] = (p3, 0.5, 'obs')
# output_color = {'r1':'red', 'r2':'green', 'r3':'red', 'null':'white'}
        ...
        for i_q in range(len(val[0])):
            ...
            for i_alpha in range(val[i_v][i_q].alpha_mat.shape[1]):
                ...
                plt.text(val[i_v][i_q].belief_points[i_alpha][1],
                         val[i_v][i_q].alpha_mat[:, i_alpha]*val[i_v][i_q].belief_points[i_alpha]+0.1,
...
#     best_e = val_new[i_v][i_q].best_edge[i_best_alpha]
...
#     (b_, o) = env.get_b_o(traj_n[-1], b, x_e_true)
#     i_o = env.x_e.index(o)
#     v_ = best_e
#     if regs[z][1]==1:
#         if regs[z][2]=='obs' or regs[z][2]=='sample':
#              q_ = np.argmax(dfsa.T(proplist[regs[z][2]])[i_q, :])
#     elif regs[z][1]>0:
#         if (regs[z][2]=='obs' or regs[z][2]=='sample') and (env.x_e[i_o] & 2**env.reg_index[z]):
#              q_ = np.argmax(dfsa.T(proplist[regs[z][2]])[i_q, :])
#     if q_ == None:
